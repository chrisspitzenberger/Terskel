...
import { deleteStepTestAction, updateStepTestAction } from "@/lib/actions/step-tests"
...
import { stepDataToStepTestStep, type StepTest } from "@/lib/db/schemas"
import { toast } from "sonner"

export function StepTestDetailClient({ test }: { test: StepTest }) {
  ...
  const handleDelete = useCallback(async () => {
    if (!test) return
    try {
      await deleteStepTestAction(test.id)
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await updateStepTestAction(test.id, { steps: updatedSteps })
      setIsEditing(false)
      setEditedSteps({})
